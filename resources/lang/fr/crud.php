<?php

return [
    "created_successfully_m" => ":Entity créé avec succès",
    "created_successfully_f" => ":Entity créée avec succès",
    "updated_successfully_m" => ":Entity mis à jour avec succès",
    "updated_successfully_f" => ":Entity mise à jour avec succès",
    "deleted_successfully_m" => ":Entity supprimé avec succès",
    "deleted_successfully_f" => ":Entity supprimée avec succès",

    "validation_error" => "Erreur de validation.",
    "creation_error" => "Erreur lors de la création de la ressource.",
    "unknown_error" => "Une erreur inconnue est survenue.",
    "sql_error" => "Erreur de base de données.",

    "slug_missing_or_invalid" => "Le slug est manquant ou invalide.",
    "not_found" => "La ressource demandée est introuvable.",
    "update_error" => "Erreur lors de la mise à jour de la ressource.",
    "delete_error" => "Erreur lors de la suppression de la ressource.",
    "forbidden" => "Action non autorisée.",
    "conflict" => "Conflit de données détecté.",
    
    
];
